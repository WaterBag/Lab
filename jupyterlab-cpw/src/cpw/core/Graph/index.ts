...
/** cpw条件节点port配置 */
export const cpwConditionPortConfig = {
  groups: {
    income: { position: 'top', attrs: { circle: { ...portAttrs.circle, style: { cursor: 'not-allowed', visibility: 'hidden' } } } },
    outgo: { position: 'bottom', attrs: portAttrs },
  },
  items: [
    { id: 'condition-income', group: 'income' },
    { id: 'condition-outgo-true', group: 'outgo' },
    { id: 'condition-outgo-false', group: 'outgo' },
  ],
}

export const CPW_NODE_SHAPE = 'cpw-cell-node'
...
//   true,
// )
export const CPW_EDGE_SHAPE = 'cpw-edge'
  ...
  const graph = new Graph({
    ...
    connecting: {
      ...
      validateMagnet({ magnet }) {
        // 如果是条件节点的income port，则不允许创建输出链接
        if (magnet.getAttribute('port') === 'condition-income') return false
        return true
      },
      validateConnection(args) {
        if (!args.sourceCell || !args.targetCell || !args.targetPort) return true

        // 不允许自己链接自己
        if (args.sourceCell.id === args.targetCell.id) return false

        /** 如果是条件节点的输出port，则不允许输入 */
        if (args.targetPort.startsWith('condition-outgo')) return false

        return true
      },
      createEdge() {
        return this.createEdge({ shape: CPW_EDGE_SHAPE, attrs: { line: { strokeDasharray: '5 5' } } })
      },
    },
    // 连接时高亮port
    highlighting: {
