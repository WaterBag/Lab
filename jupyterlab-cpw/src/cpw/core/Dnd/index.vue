...
import { computed, defineExpose, onBeforeUnmount, ref, useTemplateRef } from 'vue'
import { CPW_NODE_SHAPE, cpwConditionPortConfig, cpwPortConfig, initDnd } from '../Graph'
import { formatCellIncomes, formatCellOutgos, formatCellParams } from '../cellHandlers'
import { Button as TButton, Input as TInput, Loading as TLoading, Popup as TPopup } from 'tdesign-vue-next'
import { watchDebounced } from '@vueuse/core'
import { ChevronRightIcon, PlusIcon, RefreshIcon, StarFilledIcon, StarIcon } from 'tdesign-icons-vue-next'
...
defineExpose({
  init: (graph: Graph) => {
    ...
    startDrag = (e: MouseEvent, comp: CPW.CellComponent) => {
      ...
      e.preventDefault()
      const { key, name, desc, source, incomesConfig, outgosConfig, paramsConfig, type } = comp
      const node = graph.createNode({
        shape: CPW_NODE_SHAPE,
        ports: type === 'condition' ? cpwConditionPortConfig : cpwPortConfig,
        data: {
          key,
          name,
          desc,
  ...
  return (
    <>
      ...
      >
        ...
        <div class="cpw-dnd-category-count">{cate.children.length}</div>
      </div>
      {
        isExpanded && cate.children.map((comp, compIdx) => (
          <TPopup
            key={comp.key}
            placement="right-top"
              ...
              content: () => (
                <>
                  ...
                  <div style="display: flex; justify-content: flex-end; padding-top: 12px;">
                      ...
                      bookmark_ids.value.includes(comp.key)
                        ? (
                            <TButton
                              ...
                              variant="text"
                              size="small"
                              style="--td-text-color-primary: #cbcb4e"
                              icon={() => <StarFilledIcon />}
                    ...
                    {!bookmark && comp.genre === 'custom' && <TButton content="删除" variant="text" theme="danger" size="small" onClick={() => del(comp.key, compIdx, idx!)} />}
                  </div>
                </>
              ),
