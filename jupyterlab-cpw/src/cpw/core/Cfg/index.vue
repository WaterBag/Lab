  ...
  <div class="cpw-cfg">
    ...
    >
      ...
      <div
        ...
        "
        :style="{ marginTop: i === 0 ? '4px' : 0, marginBottom: i === syncOutgos.length - 1 ? 0 : '8px' }"
      >
        <div
    ...
    <div style="height: 1px; background-color: var(--jp-border-color2); margin: 4px 12px;" />
...
import { formatCellIncomes, formatCellOutgos, formatCellParams, paramValidator } from '../cellHandlers'
import { cloneFnJSON, useThrottleFn } from '@vueuse/core'
import type { Node } from '@antv/x6'
import type { TdCascaderProps } from 'tdesign-vue-next'
import { Dialog, showDialog, showErrorMessage } from '@jupyterlab/apputils'
import { CloseIcon, Edit2Icon, HelpCircleIcon, RollbackIcon } from 'tdesign-icons-vue-next'
import CellEditor from '../CellEditor/index.vue'

const props = defineProps<{
  activeCell: CPW.Cell
  getPredecessors: (target: Node | string) => Node[]
}>()
...
const configChanged = useThrottleFn(
  (type: ConfigType | 'all' | 'info') => {
    emit(
      'configChanged',
      props.activeCell.id,
      {
        ...((type === 'all' || type === 'params') && { params: syncParams.value.map(c => ({ ...c })) }),
        ...((type === 'all' || type === 'incomes') && { incomes: syncIncomes.value.map(c => ({ ...c })) }),
        ...((type === 'all' || type === 'outgos') && { outgos: syncOutgos.value.map(c => ({ ...c })) }),
        ...((type === 'all' || type === 'info') && syncInfo.value),
        status: 'changed',
      },
    )
  },
...
const FormLabel = ({ config, hideoptional }: { config: CPW.CellParam | CPW.CellIncome, hideoptional?: boolean }) => {
  return (
    <div style="display: flex; align-items: center; column-gap: 4px;">
      <div class="ellipsis-1">{ config.label }</div>
      { !hideoptional && !config.required && <div style="flex-shrink: 0; color: var(--td-text-color-secondary);">(可选)</div> }
      {
        config.desc && <TTooltip content={config.desc} placement="top"><HelpCircleIcon style="flex-shrink: 0; color: var(--td-brand-color); font-size: 14px;" /></TTooltip>
      }
    </div>
  )
}
...
const updateAll = (c: Omit<CPW.CellComponent, 'key' | 'genre' | 'type'>, reset = true) => {
  const { paramsConfig, incomesConfig, outgosConfig, name, desc, source } = c
  syncParams.value = formatCellParams(paramsConfig, reset ? undefined : syncParams.value)
  syncIncomes.value = formatCellIncomes(incomesConfig, reset ? undefined : syncIncomes.value)
...
defineExpose({ incomeUpdater, openEdit })
</script>
