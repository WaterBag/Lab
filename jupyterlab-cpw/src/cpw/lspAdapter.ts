...
const lspFeatures = [
  'lsp:highlights',
  'lsp:diagnostics',
  // 'lsp:syntaxHighlighting', // 这个会在输入时报错
  'lsp:codeSignature',
  'lsp:hover',
]
...
export class CPWAdapter extends WidgetLSPAdapter<CPWDocumentWidget> {
  ...
  private _injectedExtensionEditorIds = new Set<string>()
  ...
  get editors(): Document.ICodeBlockOptions[] {
    if (this.isDisposed || !this.activeEditor) return []
    return [
      {
        ceEditor: this.activeEditor,
        type: 'code',
        value: this.activeEditor!.getEditor()?.model.sharedModel.getSource() ?? '',
      },
    ]
  }

  private _activeChanged(sender: CPWDocumentWidget['content'], editor: CodeMirrorEditor | null) {
    if (!editor) return
    this._activeDocumentEditor = wrapEditor(editor)
    this._activeEditorChanged.emit({ editor: this.activeEditor! })
    editor.model.sharedModel.changed.connect(this._activeEditorContentChanged, this)
    this.updateDocuments()

    if (this._injectedExtensionEditorIds.has(editor.uuid)) return
    this._injectedExtensionEditorIds.add(editor.uuid)
    this.options.featureManager.features.forEach(feature => {
      if (!feature.extensionFactory || !lspFeatures.includes(feature.extensionFactory.name)) return
      editor.injectExtension(
        feature.extensionFactory!
          .factory({ editor: this._activeDocumentEditor, inline: true, model: editor.model, path: this.documentPath, widgetAdapter: this })!
          .instance(''),
      )
    })
  }
