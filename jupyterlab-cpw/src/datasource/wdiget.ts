...
import { KernelManager, /*  type Kernel, */ type Session, SessionManager } from '@jupyterlab/services'
import { ref } from 'vue'
...
// def __cpw_datasource_downloader(region, bucket, tmpSecretId, tmpSecretKey, sessionToken, key, dirname, filename):
//     config = CosConfig(
//         Region=region,
...
//         Bucket=bucket,
//         Key=key,
//         DestFilePath=os.path.join(os.getcwd(), dirname, filename)
//     )
// `
...
def __cpw_datasource_downloader(key, dirname, filename):
    url = '${serverOrigin}/media/data/datasource/' + key
    locale_path = os.path.join(dirname, filename)
    with requests.get(url, stream=True) as r:
        with open(locale_path, 'wb') as f:
            shutil.copyfileobj(r.raw, f)
`

const excuteCode = (task: DownloadTask) => {
  return `__cpw_datasource_downloader(key='${task.key}', dirname='${task.dirname}', filename='${task.filename}')`
}
